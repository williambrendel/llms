...
class Section extends Array {
   ...
   */
  get header() {
    return (this[0]?.hasOwnProperty("level") || (this[0]?.constructor.isHeader)) && this[0] || undefined;
  }
   ...
   */
  get content() {
    const offset = this.header && 1 || 0;
    const output = new Array(this.length - offset);
    for (let i = offset, j = 0, l = this.length; i !== l; ++i, ++j) {
      output[j] = this[i];
    }

    Object.defineProperty(output, "start", {
      get() {
        const first = this[0];
        if (!first) return undefined;
        return typeof first[0] === "number" ? first[0] : first.start;
    ...
    Object.defineProperty(output, "end", {
      get() {
        const last = this[this.length - 1];
        if (!last) return undefined;
        return typeof last[1] === "number" ? last[1] : last.end;
