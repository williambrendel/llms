 ...
 * @module scripts/io/resolveOutputPath
 ...
 *
 * Lives under `scripts/io/` because output-path layout is purely a
 * build-time concern. Nothing in `src/` decides where files land —
 * actions return Buffers; the script writes them.
 *
 * Pure function — no I/O, no side effects. The caller handles `mkdir`
 * and `writeFile`.
 */
 ...
 *                    suitable for passing to the binary pipeline.
 */
